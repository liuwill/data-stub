...
function buildTemplate(table, prefix) {
  ...
  let fileName = simpleCamelcase.toLowerCamel(template.tableName)
  ...
  if (template.tableName.startsWith(tbPrefix)) {
    fileName = simpleCamelcase.toLowerCamel(template.tableName.replace(tbPrefix, ''))
    baseClassName = simpleCamelcase.toUpperCamel(template.tableName.replace(tbPrefix, ''))
  }
  ...
  for (let i in table.fields) {
    ...
    let defineType = targetType
    if (['String', 'Number'].indexOf(defineType) > -1) {
      defineType = defineType.toLowerCase()
    }
    templateItem.targetType = targetType
    templateItem.defineType = defineType
